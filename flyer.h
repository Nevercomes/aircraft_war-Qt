...
class Flyer : public QGraphicsObject
{
    Q_OBJECT
public:
    Flyer(qreal w, qreal h, qreal speed, const QPixmaps &pixs,
          QGraphicsScene *scene, QGraphicsItem *parent = 0)
    ...
    bool checkPos(CHECK_FLAG flag)
    {
        ...
        switch (flag) {
        case UP:
            if (scenePos().ry() >= 0) ok = true;
            break;
        case DOWN:
            if (scenePos().ry() <= SIZE_HEIGHT_SCENE-m_h) ok = true;
            break;
        case LEFT:
            if (scenePos().rx() >= 0) ok = true;
            break;
        case RIGHT:
            if (scenePos().rx() <= SIZE_WIDTH_SCENE-m_w) ok = true;
            break;
        }
        return ok;
    }
